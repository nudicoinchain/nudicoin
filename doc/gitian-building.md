...
*Setup instructions for a Gitian build of Nudi Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Nudi
...
Clone the git repositories for Nudi Core and Gitian.
...
in the Nudi Core repository under 'Fetch and create inputs' to install sources which require
...
To build Nudi Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Nudi Core repository.
