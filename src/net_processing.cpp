...
#if defined(NDEBUG)
# error "Nudi Core cannot be compiled without assertions."
#endif
 ...
 *  We have 4 times smaller block times in Nudi, so we need to push 4 times more invs per 1MB. */
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        Nudi Related Inventory Messages
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // FSOCIETY this code also handles non-TXs (Dash specific messages)
