...
// Copyright (c) 2020-2023 The Nudi developers
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        {   // User-entered fsociety address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, bool fIsCoinJoin)
{
    QByteArray transaction_array; /* store serialized transaction */
    {
        ...
        for (const SendCoinsRecipient &rcp : transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal fsociety:URI (fsociety:XyZ...?message=example)
